...
export default function Home() {
  ...
  const exportToPDF = useCallback(async () => {
    ...
    pdf.text('BestMealMate - Weekly Meal Plan', pageWidth / 2, margin, { align: 'center' });
    ...
    pdf.text(dateRange, pageWidth / 2, margin + 8, { align: 'center' });
    ...
    DAYS.forEach((day, i) => {
      pdf.text(day, margin + i * colWidth + colWidth / 2, tableTop + 7, { align: 'center' });
    });

    // Meal rows
    const mealColors = {
      Breakfast: [255, 237, 213], // orange-100
      Lunch: [254, 249, 195], // yellow-100
      Dinner: [220, 252, 231], // green-100
    };

    pdf.setTextColor(0, 0, 0);
    MEALS.forEach((meal, mealIndex) => {
      const rowTop = tableTop + 10 + mealIndex * rowHeight;

      // Row background
      const color = mealColors[meal as keyof typeof mealColors];
      pdf.setFillColor(color[0], color[1], color[2]);
      pdf.rect(margin, rowTop, pageWidth - margin * 2, rowHeight, 'F');

      // Meal type label on left
      pdf.setFontSize(8);
      pdf.setTextColor(128, 128, 128);
      pdf.text(meal, margin + 2, rowTop + 6);

      // Meal content for each day
      pdf.setFontSize(9);
      pdf.setTextColor(0, 0, 0);
      DAYS.forEach((day, dayIndex) => {
        const mealName = getMeal(day, meal);
        if (mealName) {
          // Truncate long names
          const displayName = mealName.length > 15 ? mealName.slice(0, 14) + '...' : mealName;
          pdf.text(displayName, margin + dayIndex * colWidth + colWidth / 2, rowTop + 13, { align: 'center' });
        } else {
          pdf.setTextColor(180, 180, 180);
          pdf.text('-', margin + dayIndex * colWidth + colWidth / 2, rowTop + 13, { align: 'center' });
          pdf.setTextColor(0, 0, 0);
        }
      });
    });
    ...
    // Footer
    const footerY = tableTop + 10 + MEALS.length * rowHeight + 10;
    pdf.setFontSize(8);
    pdf.setTextColor(128, 128, 128);
    pdf.text(`Generated by BestMealMate on ${new Date().toLocaleDateString()}`, margin, footerY);
    pdf.text('bestmealmate.vercel.app', pageWidth - margin, footerY, { align: 'right' });

    // Nutritional summary (if meals are planned)
    const plannedMeals = Object.values(mealPlan).flatMap(day => Object.values(day)).filter(Boolean);
    if (plannedMeals.length > 0) {
      pdf.setFontSize(10);
      pdf.setTextColor(0, 0, 0);
      pdf.text(`Total meals planned: ${plannedMeals.length} / 21`, margin, footerY + 6);
    }

    // Save PDF
    pdf.save('meal-plan.pdf');
    showToast('PDF downloaded!', 'success');
  }, [getMeal, mealPlan, showToast]);
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md shadow-sm safe-top">
        <div className="max-w-6xl mx-auto px-3 md:px-4 py-2 md:py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button onClick={exportToPDF} className="p-2 rounded-full hover:bg-gray-100 active:bg-gray-200 transition-colors hidden md:flex" title="Download PDF">📄</button>
